...
fstream gradinfile("data/grad_over_time.txt", ios::in);
...
void apply_gradient() {
        ...
        for (int x=0; x<XMAX; ++x) {
          for (int y=0; y<YMAX; ++y) {
            int which = floor(ran()*YMAX);
            world[x][y].temp = grad[which];
          }
        }
}
...
void global_change() {
  for (int x = 0; x < XMAX; ++x) {
    for (int y = 0; y < YMAX; ++y) {
      world[x][y].temp += TEMP_INCR + ran()-0.5;
    }
  }
}
...
void simulate() {
        ...
        for (int t = 1; t < par.tmax+BURN_IN_TIME; t++) {
