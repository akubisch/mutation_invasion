...
fstream gradoutfile("data/grad_over_time.txt", ios::out);
...
void apply_gradient() {
        for (int x = 0; x < XMAX; x++)
                for (int y = 0; y < YMAX; y++) {
                        world[x][y].temp = par.temp_max-(par.temp_max/double(XMAX)) * double(x) + ran()-0.5;
                }
}
...
double disp_prob(int x, int y, tInd ind) {
        ...
        if (par.ddd) {
                if (ana.dens[x][y]>trait) {return(1-trait/ana.dens[x][y]); } else {return(0); }
        } else {return(trait); }
}
...
void write_grad_over_time(){
  //at first we need to find the range margin
  int R = 0;
  for (int x=0; x<XMAX-1; ++x) {
    int occx = 0;
    int occx1 = 0;
    for (int y=0; y<YMAX; ++y) {
      if ((world[x][y].females.size()+world[x][y].males.size())>0) {
        occx++;
      }
      if ((world[x+1][y].females.size()+world[x+1][y].males.size())>0) {
        occx1++;
      }
    }
    if ((occx>0)&&(occx1==0)) {
