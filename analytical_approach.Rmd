...
Temperatures occurring in the environment are denoted by $\tau_E$. The effect of temperature on survival is assumed to be of Gaussian shape:
...
To calculate the fitness effect of the given mutation strength we first calculate the product of above functions (i.e. $f(\tau_E) \cdot g(\tau_P)$), which gives us the survival probability for all occurring trait values:

\begin{equation}
h(_\tau) = \frac{1}{s_P \cdot \sqrt{2\pi}} \cdot exp \left( - \left( \frac{\tau - \mu_P}{s_P} \right)^2 - \left( \frac{\tau - \mu_E}{s_E} \right)^2 \right)
\end{equation}

We integrate $h(\tau)$ over $\tau$ to calculate mean expected fitness $\omega$:

\begin{equation}
\omega = \int_{-\infty}^{\infty} f(\tau)d\tau = \frac{1}{s_P \cdot \sqrt{2\pi}} \cdot \frac{1}{\sqrt{s^{-2}_P+s^{-2}_E}} \cdot exp \left( -\frac{(\mu_P-\mu_E)^2}{s^2_P+s^2_E} \right)
\end{equation}

The graph in Fig. 3 shows survival probability (black line) and fitness distribution at the population level (dashed red line). The highlighted area below the trait distribution indicates expected mean fitness (calculated via integration, eq. 5).

```{r Figure 3, echo=FALSE, fig.cap="Survival probability in response to temperatures $\\tau_E$ after environmental change (solid black line). The dashed red line now shows the fitness distribution within the population. The highlighted area below the trait distribution indicates expected mean fitness (calculated via integration, eq. 5). Note that for reasons of clarity the original survival probability in response to temperatures before environmental change has been omitted. Parameter values as in Fig. 2.", fig.pos="p"}
